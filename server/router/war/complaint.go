...
func (s *ComplaintRouter) InitComplaintRouter(Router *gin.RouterGroup) {
	...
	complaintPrivateRouter := Router.Group("complaint").Use(middleware.JWTAuth())
	var complaintApi = v1.ApiGroupApp.WarApiGroup.ComplaintApi
	{
	...
	{
		complaintPrivateRouter.POST("member", complaintApi.MemberComplaint) // 会员投诉
	}
}
