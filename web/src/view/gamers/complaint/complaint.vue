  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="投诉人" prop="complainant" width="120" />
        <el-table-column align="left" label="被投诉人" prop="complainee" width="120" />
        <el-table-column align="left" label="投诉原因" prop="reason" width="120" />
        <el-table-column align="left" label="投诉状态" prop="status" width="120">
          <template #default="scope">
            {{ filterDict(scope.row.status, ComplaintOptions) }}
          </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateComplaintFunc(scope.row)">变更</el-button>
...
const onDelete = async () => {
  ...
  const res = await deleteComplaintByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateComplaintFunc = async (row) => {
  const res = await findComplaint({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteComplaintFunc = async (row) => {
  const res = await deleteComplaint({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
