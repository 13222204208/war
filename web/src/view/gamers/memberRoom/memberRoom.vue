        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateMemberRoomFunc(scope.row)">变更</el-button>
...
const getTableData = async () => {
  const table = await getMemberRoomList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ...
  const res = await deleteMemberRoomByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateMemberRoomFunc = async (row) => {
  const res = await findMemberRoom({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteMemberRoomFunc = async (row) => {
  const res = await deleteMemberRoom({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
