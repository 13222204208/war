  ...
  <div>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="120px">
        ...
        <el-form-item label="红方胜场:" prop="redScore">
          <el-input v-model.number="formData.redScore" :clearable="false" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="蓝方胜场:" prop="blueScore">
          <el-input v-model.number="formData.blueScore" :clearable="false" placeholder="请输入" />
        </el-form-item>
        ...
        <el-form-item label="游戏结束时间:" prop="gameOverTime">
          <el-date-picker v-model="formData.gameOverTime" type="date" style="width:100%" placeholder="选择日期"
            :clearable="false" />
        </el-form-item> -->
      </el-form>
      <template #footer>
...
//开始游戏
const startGameRow = (row) => {
  ElMessageBox.confirm('确定要开始游戏吗?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
...
//开始游戏
const startGameFunc = async (row) => {
  const res = await startGame({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '开始成功'
