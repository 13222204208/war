  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <!-- 会员等级 -->
        <el-table-column align="left" label="会员等级" prop="memberLevelId" width="120">
          <template #default="scope">
            {{ scope.row.memberLevel.name }}
          </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="所属战队" prop="teamName" width="120" />
        <el-table-column align="left" label="按钮组" width="250">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateMemberFunc(scope.row)">变更</el-button>
            <el-button type="primary" link icon="edit" size="small" class="table-button"
              @click="toUploadImg(scope.row)">查看装备</el-button>
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateMemberMatchFunc(scope.row)">场次</el-button>
...
// ============== 表格控制部分结束 ===============
//会员等级信息
const getMemberLevelTableData = async () => {
  const table = await getMemberLevelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
    memberLevelList.value = table.data.list
  }
}

getMemberLevelTableData()
