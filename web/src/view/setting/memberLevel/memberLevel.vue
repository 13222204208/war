  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="会员名称" prop="name" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateMemberLevelFunc(scope.row)">变更</el-button>
...
// 更新行
const updateMemberLevelFunc = async (row) => {
  const res = await findMemberLevel({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.rememberLevel
...
// 删除行
const deleteMemberLevelFunc = async (row) => {
...
const enterDialog = async () => {
  elFormRef.value?.validate(async (valid) => {
    ...
    switch (type.value) {
      case 'create':
        res = await createMemberLevel(formData.value)
        break
      case 'update':
        res = await updateMemberLevel(formData.value)
        break
      default:
        res = await createMemberLevel(formData.value)
        break
